...
const DivTree: React.FC = () => {
  ...
  const addDiv = (parentId: string) => {
    if (!tree) return;
    const newDiv = createNode("Editable Text");
    const updateTree = (node: DivNode): DivNode => {
      if (node.id === parentId) {
        return { ...node, children: [...node.children, newDiv] };
      }
  ...
  const deleteDiv = (nodeId: string) => {
    if (!tree) return;
    const updateTree = (node: DivNode): DivNode | null => {
      let updatedChildren: DivNode[] = [];
      ...
      if (node.children.length !== updatedChildren.length) {
